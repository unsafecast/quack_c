...
static char* showTokenKind(QkTokKind kind) {
	...
	case QK_TOK_WHILE:  return "'while'";
        ...
        case QK_TOK_PAREN_OPEN:  return "'('";
        case QK_TOK_PAREN_CLOSE: return "')'";
        case QK_TOK_THIN_ARROW:  return "'->'";
    }
}
...
static void printToken(FILE* stream, i64 fmt, const QkToken* tok) {
    switch (tok->kind) {
	    ...
	    break;
        case QK_TOK_PAREN_OPEN:
	    qkFmtPrintf(stream, fmt, "'('");
	    break;
        case QK_TOK_PAREN_CLOSE:
	    qkFmtPrintf(stream, fmt, "')'");
	    break;
        case QK_TOK_THIN_ARROW:
	    qkFmtPrintf(stream, fmt, "'->'");
	    break;
        case QK_TOK_COMMA:
	    qkFmtPrintf(stream, fmt, "','");
	    break;
        case QK_TOK_IF:
	    qkFmtPrintf(stream, fmt, "'if'");
	    break;
        case QK_TOK_ELSE:
	    qkFmtPrintf(stream, fmt, "'else'");
	    break;
        case QK_TOK_WHILE:
